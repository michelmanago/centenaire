...
import CLASS_EFFECT, {
    classEffectArray,
    getClassForEffect,
    parseClassNameForEffect,
} from '../../components/text-editor/helpers/classEffects';
    ...
    switch (node.type) {
        ...
        case 'image':
            ...
            return `<img src="${escapeHtml(node.url)}" alt="${escapeHtml(
                node.alt,
            )}" data-legende="${legende}" data-credit="${credit}" ${
        ...
        case 'video':
            ...
            if (node.modal) {
                return `<video ${
                    node.effect ? `data-img-effect='${node.effect}'` : ''
                } class="${classNameString}" data-js-videomodal src="${escapeHtml(node.url)}"></video>`;
            }
            return `<video ${
                node.effect ? `data-img-effect='${node.effect}'` : ''
            } class="${classNameString}" controls src="${escapeHtml(node.url)}" loop></video>`;
        case 'audio':
