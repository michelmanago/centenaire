    ...
    switch (el.nodeName) {
        ...
        case 'A':
            ...
            if (el.getAttribute('data-js-pdf') != undefined) {
                return jsx(
                    'element',
                    {type: 'pdf', url: el.getAttribute('href'), dataText: el.getAttribute('data-text'), className: `${el.className}`},
                    children,
                );
            }
            return jsx(
...
const serialize = node => {
    ...
    switch (node.type) {
        ...
        case 'image':
            return `<img data-js-imagemodal src="${escapeHtml(node.url)}" alt="${escapeHtml(node.url)}" class="${classNameString}" />`;
        case 'video':
