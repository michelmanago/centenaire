...
export const deserialize = el => {
    ...
    switch (el.nodeName) {
        ...
        case 'IMG':
            ...
            blockElt.legende = el.getAttribute('data-legende');
            return jsx('element', blockElt, children);
        case 'IFRAME':
            blockElt.type = 'video';
...
const serialize = node => {
    ...
    switch (node.type) {
        ...
        case 'image':
            return `<img data-js-imagemodal src="${escapeHtml(node.url)}" alt="${escapeHtml(
                node.url,
            )}" data-legende="${escapeHtml(node.legende)}" class="${classNameString}" />`;
        case 'video':
